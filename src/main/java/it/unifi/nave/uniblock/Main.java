...
public class Main {
    ...
    private static Block initGenesis() {
        var genesisDhPk = PKHelper.generateDhKeyPair();
        var genesisSignPk = PKHelper.generateSignKeyPair();
        var genesisCertificate = Certificate.build("", genesisSignPk.getPublic(), genesisDhPk.getPublic(), Certificate.CertificateType.GENESIS);
    ...
    private static Block createUsers(String previousHash) {
    ...
    private static Certificate createUser(String name, Certificate.CertificateType certificateType) {
        ...
        var professorCertificate = Certificate.build(name, professorSignPk.getPublic(), professorDhPk.getPublic(), certificateType);
        registerPk(professorSignPk.getPrivate(), professorDhPk.getPrivate(), professorCertificate.userId());
        return professorCertificate;
    }

    private static Block publishExam(String professor, List<String> students, String previousHash) {
        var event = new Encryptable.ExamPublishing(professor, "PROGRAMMAZIONE", LocalDate.of(2017, Month.JUNE, 29));
        var eventContainer = EncryptedEvent.build(event, professor, students);
        return mineBlock(previousHash, "Publishing exam", eventContainer);
    }

    private static Block bookExam(String professor, List<String> students, String previousHash) {
    ...
    private static void registerPk(PrivateKey signPk, PrivateKey dhPk, String id) {
        PersistenceManager.getKeyManager().saveSignPk(id, signPk);
        PersistenceManager.getKeyManager().saveDhPk(id, dhPk);
    }

    private static Block mineBlock(String previousHash, String message, Event... events) {
        ...
        PersistenceManager.getBlockchain().saveBlock(block);
        return block;
    }
